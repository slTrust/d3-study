...
export default {
  ...
  methods: {
    ...
    dealData() {
      [ 'l'].forEach(direction => {
        this.root[direction] = d3.hierarchy(treeData[direction]);
        this.root[direction].x0 = this.centralPoint[0];//根节点x坐标
        this.root[direction].y0 = this.centralPoint[1];//根节点Y坐标
        this.root[direction].descendants().forEach(d => {
          d._children = d.children;//添加_children属性，用于实现点击收缩及展开功能
          d.id = direction + this.uuid();//绑定唯一标识ID
        });
        this.update(this.root[direction], direction);
      })
    },
    //画根节点
    drawRoot() {
    ...
    update(source, direction) {
      ...
      this.root[direction].eachBefore(d => {
        d.x0 = d.x;
        d.y0 = d.y;
        console.log(d.data.name,d.x0,d.y0)
      });
      ...
      const nodeEnter = node.enter().append("g")
          ...
          .attr("transform", d => `translate(${source.y0},${source.x0})`) // 因为起始点在左上角 ，移动到重置后的起点 x/x0 , y/y0 都可以
          .attr("fill-opacity", 0)
    ...
    drawTsText(id) {
      return d3.select(`#${id} text`).append('tspan')
          .attr('fill', d => this.getSubTextColor(d.parent.data.name))
          .text(d => d.data.value)
    },
    //画方框
    drawRect(id, dirRight) {
      ...
      return d3.select(`#${id}`).insert('rect', 'text')
          ...
          .attr("rx", 2) //圆角
          .style('stroke', d => this.getEndNodeRectStorke(d.parent.data.name))
          .style('fill', "#ffffff");
    },
    //画circle
    drawCircle(id) {
    ...
    //子文本颜色配置
    getSubTextColor(name) {
      let map = {'股东':'darkgray', '供应商':'#FF9800', '合伙人':'#green'};
      return map[name]?map[name]:'black'
    },
    //末 节点 边框颜色
    getEndNodeRectStorke(name) {
      let map = {'股东':'green', '供应商':'skyblue', '合伙人':'#FF9800'};
      return map[name]?map[name]:'gray'
    },
    //随机数，用于绑定id
    uuid() {
